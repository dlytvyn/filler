...
//	printf("iii %d\n", i);
//	printf("jjj %d\n", j);
//	if (i + length(piece) <= length(field) - 1 && j + width(piece) <= width(field) - 1)
//	{
//		
//		printf("connect\n");
//		return (connect);
...
int		logic(char **field, char **piece, t_lst *run)
{
	...
	while (field[i])
	{
		...
		while (field[i][j])
		{
			...
			{
				if (run->connect == -1 && run->best_x == -1 && run->best_y == -1)
				{
					run->best_x = j;
					run->best_y = i;
					run->connect = connect;
				}
				if (connect < point_num(piece) && (connect < run->connect || compare_xy(i, j, field, run)))
				{
					run->connect = connect;
					run->best_x = j;
					run->best_y = i;
